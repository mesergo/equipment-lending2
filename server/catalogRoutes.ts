...
import { createJsonStore } from './genericStore';
...
// plus Branch, Warehouse and Customer - the same JSON-file-now / MongoDB-later pattern already
// used for orders and users (see server/ordersStore.ts, server/store.ts). Moving these here (out
// of App.tsx's in-memory React state) is what makes admin CRUD on them actually survive a page
// refresh, and is a prerequisite for the "מלאי נגזר אוטומטית" (derived stock) requirement: stock
// counts are computed from SKU status server-side, never typed in by hand.

const productsStore = createJsonStore<Product>('products.json', PRODUCTS);
const modelsStore = createJsonStore<Model>('models.json', MODELS);
const branchesStore = createJsonStore<Branch>('branches.json', BRANCHES);
...
export const warehousesStore = createJsonStore<Warehouse>('warehouses.json', WAREHOUSES);
const equipmentStore = createJsonStore<EquipmentItem>('equipment.json', INITIAL_EQUIPMENT);
const customersStore = createJsonStore<Customer>('customers.json', INITIAL_CUSTOMERS);
...
catalogRouter.get('/products', (_req, res) => res.json({ products: productsStore.readAll() }));
catalogRouter.get('/models', (_req, res) => res.json({ models: modelsStore.readAll() }));
catalogRouter.get('/branches', (_req, res) => res.json({ branches: branchesStore.readAll() }));
catalogRouter.get('/warehouses', (_req, res) => res.json({ warehouses: warehousesStore.readAll() }));
catalogRouter.get('/equipment', (_req, res) => res.json({ equipment: equipmentStore.readAll() }));

// ---- Customers: contains personal info, so reading requires login too, scoped like orders. ----
catalogRouter.get('/customers', requireAuth, (req: AuthedRequest, res: Response) => {
  const auth = req.auth!;
  const all = customersStore.readAll();
...
catalogRouter.get('/customers/lookup', (req, res) => {
  const phone = String(req.query.phone || '').replace(/\D/g, '');
  if (!phone) {
    res.status(400).json({ error: 'יש לספק מספר טלפון' });
    return;
  }
  const match = customersStore.readAll().find((c) => c.mobilePhone.replace(/\D/g, '') === phone);
...
function makeCrud<T extends { id: string; organizationId?: string }>(
  ...
  router.post(path, requireAuth, (req: AuthedRequest, res: Response) => {
    const auth = req.auth!;
  ...
  router.patch(`${path}/:id`, requireAuth, (req: AuthedRequest, res: Response) => {
    const auth = req.auth!;
    const existing = store.find(req.params.id);
    if (!existing) {
    ...
    const updated = store.update(existing.id, patch as Partial<T>);
    res.json({ item: updated });
  });

  router.delete(`${path}/:id`, requireAuth, (req: AuthedRequest, res: Response) => {
    const auth = req.auth!;
    const existing = store.find(req.params.id);
    if (!existing) {
      res.status(404).json({ error: 'לא נמצא' });
